...
public class TPoseDetector : NIGestureTracker {
   ...
   /// Release the gesture
    ...
    public TPoseDetector(float timeToHoldPose, float maxMoveSpeed, float angleTolerance, float timeToSavePoints)
    {
		...
		m_pointsRightShoulder = new NITimedPointSpeedListUtility(timeToSavePoints);
        m_pointsLeftShoulder = new NITimedPointSpeedListUtility(timeToSavePoints);
    }
    ...
    public override void UpdateFrame()
    {
        if (FillPoints() == false)
            return;
        int numPoints;
        bool foundPos = TestTPose(out numPoints);
        if (numPoints < 1)
    ...
    protected bool FillPoints()
    {
        ...
        m_pointsRightShoulder.AddPoint(ref pos);
        pos = NIConvertCoordinates.ConvertPos(leftShoulder.Position);
        m_pointsLeftShoulder.AddPoint(ref pos);
        return true;
    }
    ...
    /// @note we assume the number of points is the same for all joints
    protected bool TestTPose(out int numPoints)
    {
        if (IsSteady(out numPoints) == false)
            return false;
        
		Vector3 rightShoulderPos = m_pointsRightShoulder.GetAvgPos(m_timeToHoldPose, out numPoints);
		Vector3 leftShoulderPos = m_pointsLeftShoulder.GetAvgPos(m_timeToHoldPose, out numPoints);
		...
		Vector3 shoulder = rightShoulderPos - leftShoulderPos;
		Vector3 rightArm = rightHandPos - rightElbowPos;
		Vector3 leftArm = leftHandPos - leftElbowPos;
		
		float rightArmAngle = Vector3.Angle(shoulder, rightArm);
		float leftArmAngle = Vector3.Angle(shoulder, leftArm);
		
       	//Debug.Log("RArm= " + rightArmAngle + " LArm" + leftArmAngle);
		
		if(Mathf.Abs(rightArmAngle) < m_angleTolerance && Mathf.Abs(leftArmAngle - 180) < m_angleTolerance)
		{
			//Debug.Log("T pose detected...");
			return true;	
		}
    ...
    /// this holds the points we are tracking on the right shoulder
    protected NITimedPointSpeedListUtility m_pointsRightShoulder;
    /// this holds the points we are tracking on the left shoulder
    protected NITimedPointSpeedListUtility m_pointsLeftShoulder;
