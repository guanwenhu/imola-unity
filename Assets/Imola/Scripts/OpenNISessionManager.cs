...
public class OpenNISessionManager : MonoBehaviour {
	...
	private HandsGenerator hands { get { return NIContext.Instance.CreateNode(NodeType.Hands) as HandsGenerator; }}
	private GestureGenerator gestures { get { return NIContext.Instance.CreateNode(NodeType.Gesture) as GestureGenerator; }}
	...
	void Start()
	{
		...
		if (RotateToUser) {
			if (null == userGenerator) {
				userGenerator = NIContext.Instance.CreateNode(NodeType.User) as UserGenerator;
			}
		}
	}
	...
	private DepthGenerator depthGenerator { get { return NIContext.Instance.Depth; } }
